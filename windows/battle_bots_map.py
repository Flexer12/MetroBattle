...
class BattleBotsMap(QMainWindow):
    ...
    def initUI(self):
        uic.loadUi('templates/battle_bots_map.ui', self)
        ...
        self.showPowerTeamFirstAction.triggered.connect(self.show_team_1_power)
        self.showPowerTeamSecondAction.triggered.connect(self.show_team_2_power)
    ...
    def load_map(self):
        ...
        for m in self.helpers['class_registry'].bots_maps:
            if str(self.map_id) == m.split('.')[0]:
                self.map_matrix = self.helpers['class_file'].read_file_txt(f'resources/game/bots_maps/{m}/map.csv')
                ...
                self.map_data = self.helpers['class_file'].get_file_dict(
                    f'resources/game/bots_maps/{m}/characteristic.txt')
                if self.helpers['class_file'].check_file(f'resources/game/bots_maps/{m}/music.mp3'):
                    self.helpers['class_sound'].play_music(f'resources/game/bots_maps/{m}/music.mp3')
                else:
                    print('музыки нет')
                ...
                for i in range(self.size_n):
                    for j in range(self.size_m):
                        ...
                        for group in self.groups:
                            ...
                            group_character = f'{group}'
                            print(self.map_matrix[i][j])
                            if group_character in self.map_matrix[i][j]:
                                print('Найден юнит', group_character)
                                index = self.map_matrix[i][j].index(group_character)

                                if isinstance(self.map_matrix[i][j][index], str):
                                    i_random = random.randint(0, len(
                                        self.helpers['class_registry'].characters.items()) - 1)
                                    i_find = 0
                                    character = None
                                    key_name = None
                                    for key, value in self.helpers['class_registry'].characters.items():
                                        if i_find == i_random:
                                    ...
                                    if character:
                                        self.map_matrix[i][j][index] = character
                                        self.map_matrix[i][j][index]['character'] = key_name
                ...
                if 'select_blue' in self.map_matrix[y][x]:
                    for index, character in enumerate(self.map_matrix[y][x]):
                        if (isinstance(character, dict) and character['type_object'] in self.groups and
                            ...
                            for i_action in range(self.active_action['number']):
                                if random.random() <= self.active_action['accuracy']:
                                    self.map_matrix[y][x][index]['health'] -= random.choice(
                                        self.active_action['damage'])
                    self.map_matrix[y][x].remove('select_blue')
                    if self.active_action["sound"]:
                        self.helpers['class_sound'].play_sound(self.active_action["sound"])
                    self.next = True
    ...
    def update_timer(self):
        ...
        enemy_index = self.groups[self.queue % len(self.groups) - 1]
        if len(self.characters[index]):
            ...
            if self.active_character is None:
                ...
                if not self.characters[enemy_index]:
                    self.next = True
                ...
                if 'actions' in self.active_character:
                    enemies = sorted([(self.has_path(self.active_character['j'], self.active_character['i'],
                                                     i['j'], i['i']), i, True) for i in self.characters[enemy_index]],
                ...
                if self.active_action['type'] == 'damage':
                    ...
                    enemy = min([(self.has_path(self.active_character['j'], self.active_character['i'],
                                                i['j'], i['i']), i) for i in self.characters[enemy_index]],
                                key=lambda x: len(x[0]))
                    x, y = None, None
        ...
        if self.next:
            ...
            if self.queue >= len(self.groups):
                self.queue = 0
            self.next = False
            self.set_combobox(index=0)
            self.render_map()

    # Показать силу команды 1
    def show_team_1_power(self):
        characters = {f"{enemy['character']}_{group}_{index}": enemy for group in self.groups for index, enemy in
                      enumerate(self.characters[group]) if group == 'enemy_1'}
        self.helpers['class_app'].open_power_team(characters)

    # Показать силу команды 2
    def show_team_2_power(self):
        characters = {f"{enemy['character']}_{group}_{index}": enemy for group in self.groups for index, enemy in
                      enumerate(self.characters[group]) if group == 'enemy_2'}
        self.helpers['class_app'].open_power_team(characters)
